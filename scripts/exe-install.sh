#!/usr/bin/env bash
#
# EXE installer for quickshell pill launcher.
# Handles Windows .exe files with optional Wine support,
# icon extraction, desktop entry generation, and duplicate detection.
#
# Usage:
#   exe-install.sh install <path-to.exe>
#   exe-install.sh remove  <slug>
#   exe-install.sh rename  <slug> <new name>
#
set -euo pipefail

data_home="${XDG_DATA_HOME:-$HOME/.local/share}"
apps_dir="${HOME}/.local/bin/ExeApps"
desktop_dir="$data_home/applications"
icon_dir="$data_home/quickshell/exe"
registry="$data_home/quickshell/exe.json"
...
reg_set() {
	local slug="$1" name="$2" exe="$3" icon="$4" desktop="$5" appid="$6" tmp
	tmp="$(mktemp)"
	jq --arg s "$slug" --arg n "$name" --arg a "$exe" --arg i "$icon" --arg d "$desktop" --arg p "$appid" \
		'.[$s] = {name:$n, exePath:$a, iconPath:$i, desktopPath:$d, appId:$p}' "$registry" >"$tmp"
	mv "$tmp" "$registry"
}

install_exe() {
	local src="$1"
	is_exe "$src" || die "not an exe: $src"
	...
	if command -v wine >/dev/null 2>&1; then
		if (cd "$tmp" && timeout 30 wine "$tmpcopy" --extract >/dev/null 2>&1) && [ -d "$tmp/drive_c" ]; then
			root="$tmp/drive_c"
			local df
			df="$(find "$root" -maxdepth 3 -name '*.exe' -o -name '*.inf' 2>/dev/null | head -5)"
			if [ -n "$df" ]; then
				for f in $df; do
					local n ic
					n="$(grep -m1 '^Name=' "$f" 2>/dev/null | cut -d= -f2- || true)"
					ic="$(grep -m1 '^Icon=' "$f" 2>/dev/null | cut -d= -f2- || true)"
					if [ -n "$n" ]; then
						name="$n"
						iconname="$ic"
						break
	...
	prevPath="$(jq -r --arg s "$slug" '.[$s].exePath // empty' "$registry")"
	if [ -z "$prevPath" ]; then
		action="new"
		...
		if [ -z "$prevId" ] || [ "$prevId" = "$appid" ]; then
			...
			local n=2
			while [ -n "$(jq -r --arg s "$slug-$n" '.[$s].exePath // empty' "$registry")" ]; do
				n=$((n + 1))
			done
			slug="$slug-$n"
			action="new"
...
install_icon() {
	local exe="$1" iconname="$2" slug="$3" found="" sz
	...
	if [ -n "$iconname" ]; then
		found="$(find "$exe" -path '*/scalable/*' -name "$iconname.svg" 2>/dev/null | head -1)"
		if [ -z "$found" ]; then
			for sz in 1024x1024 512x512 256x256 128x128 96x96 64x64 48x48; do
				found="$(find "$exe" -path "*/$sz/*" -name "$iconname.png" 2>/dev/null | head -1)"
				[ -n "$found" ] && break
			done
		fi
		[ -z "$found" ] && found="$(find "$exe" -name "$iconname.svg" -o -name "$iconname.png" 2>/dev/null | head -1)"
	fi

	if [ -z "$found" ] && [ -e "$exe/.DirIcon" ]; then
		found="$(readlink -f "$exe/.DirIcon" 2>/dev/null || true)"
		[ -n "$found" ] && [ -f "$found" ] || found="$exe/.DirIcon"
	fi
	[ -z "$found" ] && found="$(find "$exe" -maxdepth 1 \( -name '*.png' -o -name '*.svg' \) 2>/dev/null | head -1)"
...
remove_exe() {
	...
	exe="$(jq -r --arg s "$slug" '.[$s].exePath // empty' "$registry")"
...
cmd="${1:-}"
case "$cmd" in
	install) install_exe "${2:-}" ;;
	remove) remove_exe "${2:-}" ;;
	rename) rename_exe "${2:-}" "${3:-}" ;;
	*) die "usage: exe-install.sh install|remove|rename ..." ;;
esac